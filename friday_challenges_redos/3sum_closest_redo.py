...
def threeSumClosest( nums: list[int], target: int) -> int:
    ...
    for i, v in enumerate(nums): #takes care of i in the triplet
        ...
        while lp < rp : # pointer lp and rp will loop thru to and be added to pointer i that comes from the for loop
            ...
            current_diff = target - abs(total)

            if abs(current_diff) < abs(diff): #need abs since we need the closes number to 0 , so we make both positive to know which one is closer to that
                diff = current_diff

            if current_diff == 0:
                return target

            if current_diff > 0: #the target is too high so we need to increase total in the next iteration
                lp+=1
            else:  # the target is too low so we need to decrease the total in order for the diff to be less
                rp-=1
