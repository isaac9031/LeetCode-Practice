...
def threeSumClosest(nums: list[int], target: int) -> int:
    ...
    for i, v in enumerate(nums):
        ...
        while j < k:
            ...
            print("WITH:", abs(curr_diff), abs(diff), "WITHOUT:", curr_diff, diff )
            if abs(curr_diff) <  abs(diff): #using abs in case we have a negative number, ex. -3 is closest to 0 than 4. Making the curr_diff positive will make sure -3 is selected . 0 is the lowest difference we can have
                diff = curr_diff   #the abs also make sure that a negative is not less than all positives. ex.  -3<2, but if we use abs then 3 not less than 2
